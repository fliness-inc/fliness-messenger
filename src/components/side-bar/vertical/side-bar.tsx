...
export const SideBar: React.FC = () => {
	...
	const { data: { localState: { currentPage } } } = useQuery(gql`
		query {
			localState @client {
				currentPage
			}
	...
	const handleMenuItemClick = React.useCallback((type: PageTypeEnum, event: React.MouseEvent<HTMLButtonElement>) => {
		switch(type) {
	...
	return (
		<Grid className={classNames(classes['side-bar'], classes['side-bar_vertical'])}>
			<Grid 
				direction='column' 
			...
			<Grid justify='flex-end'>
				<MenuItem 
					Icon={SettingsIcon} 
					className={classNames(classes['item__bottom'], classes['menu__item_vertical'])}
