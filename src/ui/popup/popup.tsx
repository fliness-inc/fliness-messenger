import React, { useState, useEffect, useRef, MutableRefObject, ReactSVGElement } from 'react';
...
export const PopupContext: React.FC<PopupContextProps> = ({
    ...
    return (
        <div 
            ref={ref}
            className={classes['popup__context']} 
        >
            {children}
        </div>
    )
...
export const PopupBackground: React.FC<PopupBackgroundProps> = ({
    ...
    return (
        <div onClick={onClick} className={classes['popup__background']}>{children}</div>
    )
}
...
export const Popup: React.FC<Props> = ({
    ...
    useEffect(() => {
        if (!mounted && open) {
            ...
            setPos({ x: rect.left, y: rect.top, width: rect.width, height: rect.height })
        }
